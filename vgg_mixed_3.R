...
{ 
  ...
  all_results <- foreach(test_ind = 1:12) %dopar% {
    ...
    for (m in 1:n) {
      ...
      temp1 <- fbplot(t(y), factor = 0.1, plot = FALSE)
      resultf[m] <- ((k + 1) %in% temp1$outpoint)
      rankresultf[m] <- temp1$depth[k+1]
      
      temp2 <- detectOutlier(t(y), nCurve = k + 1, nPoint = 4096, 0.1)
  ...
  write.table(df, file = "output_vgg_mixed_1_10.csv", append = TRUE, col.names = !file.exists("output_vgg_mixed_1_10.csv"), 
              row.names = FALSE)

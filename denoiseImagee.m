...
outputFolder = 'C:\Users\harrouf\Desktop\Zine 3\Video anomaly detection\zuzheng Project\Testing\diff_at_1\Denoised_Test1';
...
for i = 1:length(imageFiles)
    ...
    imwrite(denoisedImage, outputFilePath);
    
    % Save the image as a text file
    txtFilePath = fullfile(txtOutputFolder, [imageFiles(i).name, '.txt']);
    writematrix(denoisedImage, txtFilePath, 'Delimiter', ' ');
end

disp('Denoising complete. Images saved to both image and text formats.');

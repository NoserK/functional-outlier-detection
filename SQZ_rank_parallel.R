...
all_results <- foreach(test_ind = 1:12) %dopar% {
  ...
  for (m in 1:n) {
    ...
    rankresultf[m] <- rank(temp1$depth)[k+1]
    
    temp2 <- detectOutlier(t(y), nCurve = k + 1, nPoint = 1000, 3)
...
write.table(df, file = "output_sqz_rank_of_depth.csv", append = TRUE, col.names = !file.exists("output_sqz_rank_of_depth.csv"), 
            row.names = FALSE)

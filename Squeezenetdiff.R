...
for(i in 1:16)
{
  ...
  assign(paste0("dtrain",i),
         as.matrix(get(paste0("data_train",i))[1:(k-1),])-as.matrix(get(paste0("data_train",i))[2:k,]))
  k=dim(get(paste0("data_train",i)))[1]
  y=matrix(0,nrow = k,ncol = 1000)
  ...
  all_results <- foreach(test_ind = 1:12) %dopar% {
    ...
    for (m in 1:(n-1)) {
      ...
      temp2 <- detectOutlier(t(y), nCurve = k, nPoint = 1000, 3)
  ...
  write.table(df, file = "output_Squeezenetdiff.csv", append = TRUE, col.names = !file.exists("output_Squeezenetdiff.csv"), 
              row.names = FALSE)

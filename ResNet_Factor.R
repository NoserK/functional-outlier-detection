...
for(i in 1:16)
{
  ...
  all_results <- foreach(test_ind = 1:12) %dopar% {
    ...
    for (m in 1:n) {
      ...
      temp1 <- fbplot(t(y), factor = 100, plot = FALSE)
      resultf[m] <- ((k + 1) %in% temp1$outpoint)
      rankresultf[m] <- temp1$depth[k+1]
      
      temp2 <- detectOutlier(t(y), nCurve = k + 1, nPoint = 2048, 100)
  ...
  write.table(df, file = "output_ResNet_median_100.csv", append = TRUE, col.names = !file.exists("output_ResNet_median_100.csv"), 
              row.names = FALSE)

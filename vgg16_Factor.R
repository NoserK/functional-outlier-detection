...
for(i in 1:16)
{
  ...
  #test
  extracted_features_with_prefixes_train <- read_excel("~/Desktop/fod/extracted_features_with_prefixes_train.xlsx")
  extracted_features_with_prefixes_test <- read_excel("~/Desktop/fod/extracted_features_with_prefixes_test.xlsx")
  ...
  y[1:k,]=as.matrix(get(paste0("data_train",i)))-rep(1,k)%*%t(matrix(get(paste0("mtrain",i))))+matrix(0.0001*rnorm(k*4096),nrow = k,ncol = 4096)
  ...
  write.table(df, file = "output_vgg16_median_rank.csv", append = TRUE, col.names = !file.exists("output_vgg16_median_rank.csv"), 
              row.names = FALSE)

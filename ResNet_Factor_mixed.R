...
{ 
  ...
  #test
  extracted_features_with_prefixes_train <- read_excel("~/Desktop/fod/extracted_features_with_prefixes_ResNet_train.xlsx")
  extracted_features_with_prefixes_test <- read_excel("~/Desktop/fod/extracted_features_with_prefixes_ResNet_test.xlsx")
  ...
  y[1:k,]=as.matrix(mixed_train)-rep(1,k)%*%t(matrix(get(paste0("mtrain",i))))+matrix(0.0001*rnorm(k*2048),nrow = k,ncol = 2048)
  ...
  all_results <- foreach(test_ind = 1:12) %dopar% {
    ...
    for (m in 1:n) {
      ...
      temp1 <- fbplot(t(y), factor = 5, plot = FALSE)
      resultf[m] <- ((k + 1) %in% temp1$outpoint)
      rankresultf[m] <- temp1$depth[k+1]
      
      temp2 <- detectOutlier(t(y), nCurve = k + 1, nPoint = 2048, 5)
  ...
  write.table(df, file = "output_ResNet_mixed_5.csv", append = TRUE, col.names = !file.exists("output_ResNet_mixed_5.csv"), 
              row.names = FALSE)

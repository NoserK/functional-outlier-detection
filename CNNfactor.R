...
for(i in 1:5)
{
  ...
  j=2
  #for(j in 1:2){
    n=dim(get(paste0("data_Test",j)))[1]
    assign(paste0("resultd",i,j),as.vector(rep(FALSE,n)))
    assign(paste0("resultf",i,j),as.vector(rep(FALSE,n)))
    
    vector_name1 <- paste0("resultf", i, j)
    temp_vector1 <- get(vector_name1)
    vector_name2 <- paste0("resultd", i, j)
    temp_vector2 <- get(vector_name2)
    for(m in 1:n)
    {
      
      y[k+1,]=as.matrix(get(paste0("data_Test",j))[m,])-get(paste0("mTest",j))+0.0001*rnorm(2048,0,1)
      temp1=fbplot(t(y),factor = 3,plot = F)
      temp_vector1[m]=((k+1) %in% temp1$outpoint)
      temp2=detectOutlier(t(y),nCurve = k+1,nPoint = 2048,3)
      temp_vector2[m]=((k+1) %in% temp2$outlier)
    }
    assign(vector_name1, temp_vector1)
    assign(vector_name2, temp_vector2)
    cat("Finish loop of train", i, "test", j, "\n")
  #}
  # AUCROC
  ...
  vector_name3 <- paste0("tpr", 4)
  temp_vector3 <- get(vector_name1)
  vector_name4 <- paste0("fpr", i)
  temp_vector4 <- get(vector_name4)
  #for (j in 1:2){
  temp_vector3[j] <- sum(get(paste0("resultd",i,j))[groundTruth[[j]]] == TRUE) /
    length(groundTruth[[j]])
    
    if (length(groundTruth[[j]]) == 
        length(get(paste0("resultd",i,j)))){
      temp_vector4[j] <- 1
    } else {
      temp_vector4[j] <- sum(get(paste0("resultd",i,j))[-groundTruth[[j]]] == TRUE) /
        (length(get(paste0("resultd",i,j))) - length(groundTruth[[j]]))
    }
  #}
  assign(vector_name3, temp_vector3)
  assign(vector_name4, temp_vector4)
  df=data.frame(c(get(paste0("tpr",i)),get(paste0("fpr",i))))
  
  write.table(df, file = "output.csv", append = TRUE,  row.names = FALSE)

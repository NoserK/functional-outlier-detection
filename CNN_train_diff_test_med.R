...
for(i in 1:16)
{
  ...
  all_results <- foreach(test_ind = 1:12) %dopar% {
    ...
    for (m in 1:n) {
      y[k, ] <- as.matrix(get(paste0("data_Test", test_ind))[m, ]) - 
  ...
  write.table(df, file = "output_cnn_train_diff_test_med.csv", append = TRUE, col.names = !file.exists("output_cnn_train_diff_test_med.csv"), 
              row.names = FALSE)

...
for(i in 1:16)
{
  ...
  all_results <- foreach(test_ind = 1:12) %dopar% {
    ...
    for (m in 1:n) {
      ...
      temp1 <- fbplot(t(y), factor = 4, plot = FALSE)
      resultf[m] <- ((k + 1) %in% temp1$outpoint)
      
      temp2 <- detectOutlier(t(y), nCurve = k + 1, nPoint = 1000, 4)
  ...
  write.table(df, file = "output4.csv", append = TRUE, col.names = !file.exists("output4.csv"), 
              row.names = FALSE)
